...
namespace UnityUtils
{
    public static class GUIHelper
    {
        ...
        /// <summary>
        /// Draw a label with stroke
        /// </summary>
        /// <param name="rect">The rectangle to draw the label in</param>
        /// <param name="text">The text of the label</param>
        /// <param name="style">The GUIStyle to use for the label</param>
        /// <param name="borderColor">The color of the border</param>
        /// <param name="borderWidth">The width of the border</param>
        public static void LabelWithBorder(Rect rect, string text, GUIStyle style, Color borderColor, int borderWidth = 2)
        {
            // Create a new GUIStyle for the border based on the original style
            GUIStyle borderStyle = new(style);
            borderStyle.normal.textColor = borderColor;
            ...
            for (int i = -borderWidth; i <= borderWidth; i++)
            {
                ...
                {
                    GUI.Label(new Rect(rect.x + i, rect.y + j, rect.width, rect.height), text, borderStyle);
                }
            }
        ...
        public static void DrawLine(Vector2 lineStart, Vector2 lineEnd, Color color, int thickness)
        {
            if (_coloredLineTexture == null || _coloredLineColor != color)
            {
                ...
                {
                    Object.Destroy(_coloredLineTexture);
                    _coloredLineTexture = null;
                }
        ...
        public static void DrawLine(Vector2 lineStart, Vector2 lineEnd, Texture2D texture, int thickness)
        {
            ...
            // regardless of the thickness.
            int thicknessOffset = (int)Mathf.Ceil(thickness / 2);
            Rect drawingRect = new(lineStart.x, lineStart.y - thicknessOffset, Vector2.Distance(lineStart, lineEnd), (float)thickness);
            GUIUtility.RotateAroundPivot(angle, lineStart);
